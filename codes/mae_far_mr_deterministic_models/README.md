...
This Python script computes Mean Absolute Error (MAE), False Alarm Rate (FAR), and Miss Rate (MR) for monsoon onset predictions from deterministic forecast. It processes multiple years of model forecast data (merged and regridded as individual files for each year) and IMD (India Meteorological Department) observational data to evaluate model performance.
...
python mae_far_mr_deterministic_models.py \
    ...
    --forecast_days 15 \
    --mok \
    --output_file results_1-15day_MOK.nc \
...
python mae_far_mr_deterministic_models.py \
    ...
    --forecast_days 30 \
    --mok \
    --output_file results_16-30day_MOK.nc \
    --plot_dir /output/plots
```
...
python mae_far_mr_deterministic_models.py \
    ...
    --verification_window 1 \
    --forecast_days 15 \
    --output_file results_1-15day_noMOK.nc
```
